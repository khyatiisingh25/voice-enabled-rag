...
The timeout behavior is enforced at the API layer with a 5-second limit. No production timeout configuration was changed as part of this analysis.

## Recommendation

The primary optimization target is query embedding. FAISS retrieval and deterministic fallback are already negligible contributors to request latency, so optimization should focus on reducing embedding computation time without changing retrieval behavior or answer-selection logic until quality impact is measured.
